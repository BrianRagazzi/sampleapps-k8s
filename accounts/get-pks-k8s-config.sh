...
#TOKENS=$(curl 'https://${PKS_API}:8443/oauth/token' -sk -X POST -H 'Accept: application/json' -d \"client_id=pks_cluster_client&client_secret=\"\"&grant_type=password&username=${PKS_USER}&password=\"${PKS_PASSWORD}\"&response_type=id_token\")
#TOKENS=$(eval $CURL_CMD | jq -r '{id_token, refresh_token} | to_entries | map("\(.key)=\(.value | @sh)") | .[]')
id_token=$(eval $CURL_CMD | jq -r '.id_token')
refresh_token=$(eval $CURL_CMD | jq -r '.refresh_token')
#echo $TOKENS
# id_token=$(cat $TOKENS | jq -r '.id_token')
# refresh_token=$(cat $TOKENS | jq -r '.refresh_token')
#echo $id_token
